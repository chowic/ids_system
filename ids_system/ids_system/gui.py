...
class AlertTableModel(QAbstractTableModel):
    ...
    def get_alert_at(self, row):
        if 0 <= row < len(self.filtered_alerts):
            return self.filtered_alerts[row]
        return None
...
class CaptureThread(QThread):
    traffic_signal = pyqtSignal(int, int)
...
class IDSGui(QMainWindow):
    ...
    def on_alert_updated(self, index):
        self.table_model.update_row(index)

    def _refresh_table(self):
        if self.table_model and hasattr(self, 'table_view'):
