...
def test_sql_injection():
    print("[*] 测试 SQL注入...")
    payload = b"' OR '1'='1"
    pkt = IP(dst="127.0.0.1") / TCP(dport=80, flags="PA") / Raw(load=payload)
...
def test_bruteforce():
    ...
    for i in range(10):
        ...
        pkt = IP(dst="127.0.0.1") / TCP(dport=22, flags="PA") / Raw(load=payload)
        send(pkt, verbose=False)
        time.sleep(0.05)
...
def test_bandwidth():
    ...
    for i in range(100):  # 发送100个包，约150KB
        pkt = IP(dst="127.0.0.1") / TCP(dport=80, flags="PA") / \
...
def test_tls_malicious():
    print("[*] 测试 TLS恶意域名识别...")
    # 模拟一个十六进制的标准 TLS ClientHello 头部片段 (包含握手特征 0x16 0x03)
    # 里面硬编码嵌入了 "ngrok.io" 作为 SNI 扩展特征，与你的 config.py 黑名单相匹配
    target_domain = "ngrok.io"
    fake_tls_client_hello = (
        b"\x16\x03\x01\x00\xba\x01\x00\x00\xb6\x03\x03"
        b"\x00\x00\x00\x00"  # 随机数占位
        b"\x00"              # Session ID 长度
        b"\x00\x02\x00\x2f"  # 加密套件
        b"\x01\x00"          # 压缩方法
        b"\x00\x8b"          # 扩展长度
        b"\x00\x00\x00\x0c\x00\x0a\x00\x00\x09" + target_domain.encode('utf-8')
    )
    try:
        # 发送包含 TLS 特征的测试包到 8.8.8.8 的 443 端口
        pkt = IP(dst="127.0.0.1") / TCP(dport=443, flags="PA") / \
            Raw(load=fake_tls_client_hello)
        send(pkt, verbose=False)
        print("[✓] TLS恶意域名测试完成")
    except Exception as e:
        print(f"[!] TLS测试包发送失败: {e}")


def test_all():
    while True:
        # while 循环体内的所有内容都需要向右缩进
        print("\n" + "=" * 50)
        print("开始测试所有功能")
        print("=" * 50 + "\n")
...
if __name__ == "__main__":
    while True:
        ...
        if choice == "1":
            ...
            test_tls_malicious()  # 触发单次 TLS 测试
        elif choice == "0":
            print("[*] 正在退出测试工具...")
            break
        else:
